...
**Ключевые сценарии:**
1. **Каталог (Feed)** — единый запрос `/feed/` возвращает карточки из 3 источников: номенклатура (stock_material + service), готовые букеты (finished_bouquet batches), активные резервы.
2. **Checkout** — атомарная транзакция: создание Sale + SaleItems + FIFO-списание + StockMovement + SaleItemComposition (для букетов, из snapshot) + финансовая транзакция. Ошибки нехватки остатков и отсутствия позиций корректно возвращают 400 (не 500).
...
7. **Услуги** — позиции с `accounting_type=service` не показывают остаток, доступны для добавления без ограничения количества.
8. **Контроль смены** — кассовый интерфейс заблокирован, если в торговой точке нет открытой кассовой смены (GET `/finance/cash-shifts/?status=open`).
9. **Интеграция с Продажами** — все продажи через кассу автоматически попадают в блок «Продажи» → «Список продаж».
...
## Changelog (2026-03-27) — Переработка модуля «Склад»: остатки, поступления, букеты

### Backend
- ✅ Исправлена ошибка сохранения документов приёмки «items: [object Object]» — поле `document` добавлено в `read_only_fields` у `ReceiptDocumentItemSerializer`
- ✅ Сборка букета (`assemble_bouquet`) теперь использует `MovementType.ASSEMBLY` вместо `RECEIPT` — собранные букеты больше не создают ложных поступлений
- ✅ При сборке на партию устанавливается `is_assembly=True`
- ✅ `BatchViewSet` исключает сборочные партии (`is_assembly=True`) из листинга
- ✅ `StockBalanceSerializer` расширен полями `purchase_price`, `retail_price`, `accounting_type` (из номенклатуры)
- ✅ `StockBalanceViewSet.destroy()` — удаление остатков только для ролей owner/admin (403 для остальных)
- ✅ `StockBalanceViewSet.bouquet_detail()` — drill-down эндпоинт для просмотра состава букета: партия, сборщик, снимок компонентов, шаблон

### Frontend (`InventoryPage.tsx`)
- ✅ Вкладка «Партии» удалена — партии теперь отображаются внутри вкладки «Поступления» (read-only)
- ✅ Вкладка «Остатки»: столбцы «Ср. цена» и «Сумма» заменены на «Цена закупа» и «Цена продажи»
- ✅ Поле «Обновлено» показывает только дату (без времени)
- ✅ Кнопка удаления остатков — отображается только для admin/owner
- ✅ Кнопка «Просмотр» для букетов — открывает детальный диалог с составом, фото, ценами и кнопкой «Коррекция»
- ✅ Галочка «Сохранить в шаблоны» добавлена в режиме индивидуального букета
- ✅ Автокомплит шаблонов в режиме «По шаблону» использует `bouquetTemplates` (из раздела «Шаблоны букетов»)
