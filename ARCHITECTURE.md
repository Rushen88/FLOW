...
│   │   ├── App.tsx                 # Маршрутизация (14 маршрутов + PrivateRoute)
...
| **Nomenclature** | Товар | group → NomenclatureGroup, type (12 типов), **accounting_type** (stock_material/finished_bouquet/service), sku, purchase_price, retail_price, min_price, color, country, season, shelf_life, min_stock |
...
| Модель | Описание | Ключевые поля |
|--------|----------|---------------|
| **Batch** | Партия товара | nomenclature → Nomenclature, supplier → Supplier, warehouse → Warehouse, purchase_price, quantity, remaining, expiry_date, **receipt_document → ReceiptDocument**, **is_assembly** |
...
- Сборка букета: `assemble_bouquet()` → BouquetBatchComponentSnapshot (фиксация состава для аудита)
...
| inventory | `/api/inventory/` | batches, stock (readonly, /summary, /low-stock, /negative-alerts), movements, inventory-documents, inventory-items, reserves, **receipt-documents** (/process) |
